...
const APPEARANCE_KEY = "finfit.appearance.v6";
const SPORT_FILTER_KEY = "finfit.sportFilter.v1";
const DB_NAME = "finfit-db";
const DB_VERSION = 1;

const { presets, activityContexts, weekDays, foodCatalog, dailyCheckItems, dailyFoodItems, dailyBodyItems, baseTemplates } = window.FinfitCatalogs;
...
function applyAppearance(appearance = loadAppearance()) {
  const theme = ["black", "light"].includes(appearance.theme) ? appearance.theme : "black";
  const accent = ["lime", "teal", "purple", "amber"].includes(appearance.accent) ? appearance.accent : "purple";
  document.documentElement.dataset.theme = theme;
  document.documentElement.dataset.accent = accent;
  const meta = $("#themeColorMeta");
  if (meta) meta.content = theme === "black" ? "#101010" : "#f4f4f1";
  const themeButton = $("#themeToggleButton");
  if (themeButton) themeButton.textContent = theme === "black" ? "☀️" : "🌙";
...
$("#themeToggleButton").addEventListener("click", () => {
  ...
  const next = current.theme === "black" ? "light" : "black";
